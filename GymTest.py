...
def q_network(X_state, name):
    ...
    #I believe that outputs is the network, which has 3 conv layers and 2 fully connected layers
    return outputs, trainable_vars_by_name

X_state = tf.placeholder(tf.float32, shape=[None, input_height, input_width,
                                            input_channels])
#the networks for the online and target q values
...
#this is the function that decides whether or not to do a random action or an optimal action, makes exploration emphasized
    ...
    with tf.variable_scope(scope_name):
        for i, size in enumerate(layers_sizes):
            inputs = tf.layers.dense(
                inputs,
                size,
                activation=tf.relu if i < len(layers_sizes) - 1 else None,
                kernal_initializer=tf.contrib.layers.xavier_initializer(),
                name=scope_name+"_1"+str(i)
            )
    return inputs
...
actions = tf.placeholder(tf.float32, shape=(batch_size,),name='action')
rewards = tf.placeholder(tf.float32, shape=(batch_size, observation_size),name='reward')
done_flags = tf.placeholder(tf.float32, shape=(batch_size, observation_size),name='done')

q = dense_nn(states, [32,32,2],name="Q_primary")
q_target - dense_nn(states_next, [32,32,2],name='Q_target')


for i_episode in range(5):
    observation = env.reset() #this gets an initial observation,
    for t in range(1000):
        env.render()
        #print(observation)
        action = env.action_space.sample()
        observation, reward, done, info = env.step(action) #takes a random action,returns the obervation, reward, done and info

        #info is used for debugging
        if done:
